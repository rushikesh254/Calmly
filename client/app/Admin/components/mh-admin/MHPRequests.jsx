...
/**
 * Mental Health Professional (MHP) registration request review component.
 * Fetches pending professional accounts and lets an MH Admin approve / reject.
 * UI styles are kept identical; only structure & comments improved.
 */
export function MHPRequest() {
	...
	useEffect(() => {
		const fetchPendingMHPs = async () => {
			try {
				...
				const response = await fetch(`${API_BASE}/api/mhps/requests`, {
					headers: token ? { Authorization: `Bearer ${token}` } : {},
				});
				if (!response.ok) throw new Error("Failed to fetch pending MHP requests");
	...
	const handleDecision = async (userName, userEmail, decision) => {
		const safeUserName = userName || ""; // Guard for undefined
		try {
			const token = localStorage.getItem("accessToken") || localStorage.getItem("token");
			const res = await fetch(`${API_BASE}/api/mhps/status/${safeUserName}`, {
				method: "PUT",
				headers: {
	...
	return (
		<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
			...
			<div className="mb-8 text-center">
				<h2 className="text-4xl font-bold bg-gradient-to-r from-indigo-600 to-teal-500 bg-clip-text text-transparent mb-2">MHP Registration Requests</h2>
				<p className="text-gray-600">Review and manage mental health professional applications</p>
			</div>
							...
							<div className="flex flex-col md:flex-row justify-between items-start gap-6">
								<div className="space-y-3 flex-1">
									<div className="flex items-center justify-between">
										<h3 className="text-xl font-semibold text-gray-800">{mhp.username || mhp.userName}</h3>
										<span className="px-3 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-800">Pending Review</span>
									</div>
									...
									<div className="space-y-2">
										...
										<div className="flex items-center">
											...
											<span className="text-gray-600">Licence: {mhp.licenseNumber || mhp.bmdcRegNo}</span>
										</div>
									</div>
								</div>
								...
								<div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
									<Button
										onClick={() => handleDecision(mhp.username || mhp.userName, mhp.email, DECISION.APPROVE)}
									...
									<Button
										onClick={() => handleDecision(mhp.username || mhp.userName, mhp.email, DECISION.REJECT)}
