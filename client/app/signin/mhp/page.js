...
export default function MHPSignInPage() {
	...
	return (
		<div className="min-h-dvh bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 flex items-center justify-center p-6 overflow-x-hidden">
			...
			{/* Sign-in card */}
			<motion.div variants={containerMotion} initial="hidden" animate="visible" className="w-full max-w-md z-10">
				<motion.div variants={itemMotion}>
					<Card className="p-8 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm border-0 shadow-2xl">
						<motion.div variants={itemMotion} className="text-center mb-8">
							...
							<h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Professional Sign In</h1>
							<p className="text-slate-600 dark:text-slate-300">Access your MHP dashboard and manage your clients</p>
						</motion.div>

						<AnimatePresence>
							{errorMessage && (
								<motion.div initial={{ opacity: 0, y: -10, height: 0 }} animate={{ opacity: 1, y: 0, height: "auto" }} exit={{ opacity: 0, y: -10, height: 0 }} transition={{ duration: 0.3 }} className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg flex items-center space-x-3">
						...
						<form onSubmit={handleFormSubmit} className="space-y-6">
							...
							<motion.div variants={itemMotion}>
								...
								<div className="relative mt-2">
									...
									<Input
										...
										required
									/>
									<motion.button type="button" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setIsPasswordVisible(!isPasswordVisible)} className={eyeToggleClass}>
							...
							<motion.div variants={itemMotion}>
								<motion.button type="submit" disabled={loading} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className={submitButtonClass}>
									{loading ? (
						...
						<motion.div variants={itemMotion} className="mt-8 text-center space-y-4">
							<div className="flex items-center justify-center space-x-2 text-sm">
								<span className="text-slate-600 dark:text-slate-400">Forgot your password?</span>
								<Link href="/forgot-password" className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-medium transition-colors">
									Reset Password
								</Link>
							</div>
						</motion.div>
			...
			<AnimatePresence>
				{modalOpen && (
					<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-6" onClick={() => setModalOpen(false)}>
						<motion.div variants={modalMotion} initial="hidden" animate="visible" exit="exit" onClick={(event) => event.stopPropagation()} className="bg-white dark:bg-slate-800 rounded-2xl p-8 max-w-md w-full shadow-2xl">
							<div className="flex items-center space-x-4 mb-6">
								<div className={`w-12 h-12 rounded-full flex items-center justify-center ${hasError ? "bg-red-100 dark:bg-red-900/20" : "bg-green-100 dark:bg-green-900/20"}`}>
									{hasError ? (
								...
								<div>
									<h3 className={`text-lg font-semibold ${hasError ? "text-red-900 dark:text-red-100" : "text-green-900 dark:text-green-100"}`}>
										{hasError ? "Error" : "Success"}
									</h3>
									<p className={`text-sm ${hasError ? "text-red-600 dark:text-red-300" : "text-green-600 dark:text-green-300"}`}>
										{statusMessage}
									</p>
								</div>
							</div>
							<motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setModalOpen(false)} className="w-full bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-900 dark:text-white font-medium py-3 px-6 rounded-lg transition-colors duration-200">
								Close
							</motion.button>
						</motion.div>
					</motion.div>
