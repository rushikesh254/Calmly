...
import { Heart, Eye, EyeOff, Lock, Mail, AlertCircle, CheckCircle, Shield } from "lucide-react";
...
export default function MHPSignInPage() {
	...
	const handleSubmit = async (e) => {
		...
		setSubmitting(true);
		try {
			const res = await fetch(`${apiBase}/api/mhps/signin`, {
				method: "POST",
	...
	// Animation variants
	const container = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut", staggerChildren: 0.1 } } };
			...
			<AnimatePresence>
				{modalOpen && (
					<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-6" onClick={() => setModalOpen(false)}>
						<motion.div variants={modalVariants} initial="hidden" animate="visible" exit="exit" onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-slate-800 rounded-2xl p-8 max-w-md w-full shadow-2xl">
							<div className="flex items-center space-x-4 mb-6">
								<div className={`w-12 h-12 rounded-full flex items-center justify-center ${modalIsError ? "bg-red-100 dark:bg-red-900/20" : "bg-green-100 dark:bg-green-900/20"}`}>
									{modalIsError ? <AlertCircle className="w-6 h-6 text-red-500" /> : <CheckCircle className="w-6 h-6 text-green-500" />}
								</div>
								<div>
									<h3 className={`text-lg font-semibold ${modalIsError ? "text-red-900 dark:text-red-100" : "text-green-900 dark:text-green-100"}`}>{modalIsError ? "Error" : "Success"}</h3>
									<p className={`text-sm ${modalIsError ? "text-red-600 dark:text-red-300" : "text-green-600 dark:text-green-300"}`}>{statusMessage}</p>
								</div>
							</div>
							<motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setModalOpen(false)} className="w-full bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-900 dark:text-white font-medium py-3 px-6 rounded-lg transition-colors duration-200">Close</motion.button>
						</motion.div>
					</motion.div>
				)}
			</AnimatePresence>
