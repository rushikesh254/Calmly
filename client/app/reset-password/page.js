...
export default function ResetPassword() {
	...
	const onSubmit = async (e) => {
		...
		try {
			...
			if (!res.ok) throw new Error("Error updating password. Please try again.");
	...
	<div className="min-h-dvh bg-gradient-to-br from-slate-50 via-blue-50 to-teal-75 overflow-x-hidden">
			...
			{modalOpen && (
				<div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" role="dialog" aria-modal="true">
					<div className={`p-6 rounded-lg shadow-xl ${modalError ? "bg-red-50 border border-red-200" : "bg-green-50 border border-green-200"}`} role="alert" aria-live="polite">
						<p className={`text-center mb-4 ${modalError ? "text-red-600" : "text-green-600"}`}>{modalMsg}</p>
					</div>
				</div>
			)}
	...
	<main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
		  ...
		  <Card className="w-full max-w-md p-8 bg-white/90 backdrop-blur-sm border border-slate-200/60 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
				...
				<form onSubmit={onSubmit} className="space-y-6" noValidate>
							...
							<Input type="password" placeholder="Enter new password" value={password} onChange={(e) => setPassword(e.target.value)} className="bg-white/50 focus:bg-white/70 border-slate-200/60 focus:ring-2 focus:ring-indigo-500/50" required aria-required="true" />
									</div>
								</div>

						<Button type="submit" className="w-full bg-gradient-to-r from-indigo-600 to-teal-500 text-white hover:from-indigo-700 hover:to-teal-600 transition-all transform hover:scale-[1.02] shadow-lg hover:shadow-indigo-200/40" disabled={submitting} aria-busy={submitting}>{submitting ? "Saving..." : "Save Password"}</Button>
							</form>
