...
export const DashboardOverview = ({ role, userName }) => {
	...
	// Role flags were unused; removed to satisfy linter
	...
	// Local dynamic state (will override static placeholders when realtime data arrives)
	const [liveStats, setLiveStats] = useState(null);
	const esRef = useRef(null);

	// Static dashboard data per role – used until we fetch realtime values
	const stats = {
		attendee: [
			{
				title: "Upcoming Sessions",
				value: liveStats?.upcomingSessions?.toString() || "2",
			...
			},
			{
				...
				value: liveStats?.resourcesViewed?.toString() || "12",
			...
			{
				title: "Mood Entries",
				value: liveStats?.moodEntries?.toString() || "28",
			...
			},
			{
				...
				value: liveStats?.journalEntries?.toString() || "15",
	...
	// Establish SSE stream for attendee role
	useEffect(() => {
		if (role !== "attendee" || !userName) return;
		const email = encodeURIComponent(
			userName.includes("@") ? userName : "demo.user@example.com"
		);
		const candidates = [
		...
		const tryConnect = (idx = 0) => {
			if (connected || idx >= candidates.length) return;
			const base = candidates[idx];
			const url = `${base}/api/dashboard/attendee/stream?email=${email}`;
			...
			es.onerror = () => {
				if (!connected) {
					es.close();
					tryConnect(idx + 1); // next candidate
				} else {
					// if previously connected, attempt soft reconnect
					es.close();
					setTimeout(() => tryConnect(0), 5000);
				}
			};
		};
	...
	const handleQuickActionClick = useCallback((target) => {
		if (typeof window === "undefined") return;
		try {
			const url = new URL(target, window.location.origin);
			const currentPath = window.location.pathname;
			if (url.pathname === currentPath && url.hash) {
				const nextSection = url.hash.slice(1);
				if (nextSection) window.location.hash = nextSection;
				return;
			}
	...
	return (
		<div className={wrapperClass} aria-labelledby="dashboard-welcome-heading">
			...
			<div
				...
				role="list"
				aria-label="High level statistics">
			...
			<div aria-labelledby="quick-actions-heading">
				<h2 id="quick-actions-heading" className={quickTitleClass}>
					Quick Actions
				</h2>
				<div
			...
			<div aria-labelledby="recent-activity-heading">
				<h2 id="recent-activity-heading" className={recentTitleClass}>
					Recent Activity
				</h2>
