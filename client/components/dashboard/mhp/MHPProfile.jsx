...
const MHPProfile = ({ userName }) => {
  ...
  const handleDayToggle = useCallback((rosterType, day, isChecked) => {
    setProfileData((prev) => {
      ...
      if (isChecked) roster[day] = roster[day] || "";
      else delete roster[day];
  ...
  return (
    <div
      className="bg-white/90 backdrop-blur-lg p-8 rounded-2xl border border-slate-200/60 shadow-xl shadow-indigo-100/20 hover:shadow-2xl transition-shadow duration-300"
      aria-labelledby="mhp-profile-heading"
    >
      <h3
        id="mhp-profile-heading"
        className="text-2xl font-bold bg-gradient-to-r from-indigo-600 to-teal-500 bg-clip-text text-transparent mb-8"
      >
        MHP Profile
      </h3>
      {isEditing ? (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-6">
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-slate-200/60 rounded-xl bg-white/70 focus:ring-2 focus:ring-indigo-200/80 focus:border-indigo-300/50 transition-all outline-none placeholder:text-slate-400/80"
              ...
              <div className="space-y-4">
                {days.map((day) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      className="h-5 w-5 text-indigo-600 rounded-lg border-2 border-slate-300 checked:bg-indigo-600 checked:border-indigo-600 focus:ring-indigo-200/80 transition-all"
                      onChange={(e) =>
                        handleDayToggle("rosterOnline", day, e.target.checked)
                      }
                      checked={profileData.rosterOnline[day] !== undefined}
                      aria-label={`Toggle online availability for ${day}`}
                    />
                    ...
                    {profileData.rosterOnline[day] !== undefined && (
                      <input
                        ...
                        className="flex-1 px-4 py-2.5 border border-slate-200/60 rounded-lg bg-white/70 focus:ring-2 focus:ring-indigo-200/80 focus:border-indigo-300/50 transition-all outline-none placeholder:text-slate-400/80 text-sm"
                      />
                    )}
                  </div>
                ))}
                ...
                {days.map((day) => (
                  ...
                  >
                    <input
                      ...
                      checked={profileData.rosterOffline[day] !== undefined}
                      aria-label={`Toggle offline availability for ${day}`}
                    />
                    <span className="w-20 font-medium text-slate-700">
                      {day}
                    </span>
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="w-full px-6 py-3 bg-gradient-to-r from-indigo-600 to-teal-500 text-white rounded-lg font-semibold hover:from-indigo-700 hover:to-teal-600 transition-all transform hover:scale-[1.02] shadow-md hover:shadow-indigo-200/40 active:scale-95 text-sm"
            aria-busy={saving}
          >
