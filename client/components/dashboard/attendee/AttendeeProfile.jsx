...
const fetchProfile = async (userName) => {
	const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/attendees/${userName}`);
...
const classes = {
	...
	title: "text-2xl font-bold bg-gradient-to-r from-indigo-600 to-teal-500 bg-clip-text text-transparent mb-8",
	...
	inputBase: "w-full px-4 py-3 border border-slate-200/60 rounded-xl bg-white/70 focus:ring-2 focus:ring-indigo-200/80 focus:border-indigo-300/50 transition-all outline-none",
	primaryBtn: "w-full mt-6 px-6 py-3 bg-gradient-to-r from-indigo-600 to-teal-500 text-white rounded-lg font-semibold hover:from-indigo-700 hover:to-teal-600 transition-all transform hover:scale-[1.02] shadow-md hover:shadow-indigo-200/40 active:scale-95 text-sm",
};
			...
			<div className={classes.card}>
				...
				{editing ? (
					<div className="space-y-6">
						...
						<button onClick={onSave} className={classes.primaryBtn}>Save Changes</button>
					</div>
				) : (
					<div className="space-y-6">
						...
						<button onClick={() => setEditing(true)} className={classes.primaryBtn}>Edit Profile</button>
					</div>
				)}
			</div>
		</div>
